...
const getAllPublicTournaments = async (req, res) => {
  try {
    // Fetch only verified tournaments
    const tournaments = await Tournament.find({ isVerified: true });
    console.log("Verified tournaments found:", tournaments.length);
...
const getTournamentPlayers = async (req, res) => {
  try {
    const { id } = req.params;
    console.log('Fetching players for tournament ID:', id);

    // Get all individual players
    const TeamIndividual = (await import('../../Models/Organizer/TeamIndividual.js')).default;
    const TeamGroup = (await import('../../Models/Organizer/TeamGroup.js')).default;

    // Find all individual players in the tournament
    const individualPlayers = await TeamIndividual.find({ tournamentId: id })
      .select('name email mobile academyName event')
      .lean();

    console.log('Individual players found:', individualPlayers.length);

    // Find all group teams in the tournament
    const groupTeams = await TeamGroup.find({ tournamentId: id })
      .select('teamName members.name members.email members.mobile members.academyName event')
      .lean();

    console.log('Group teams found:', groupTeams.length);
    ...
    console.log('Total players to return:', players.length);
    return res.json({ success: true, players });
  } catch (error) {
    console.error('Error fetching tournament players:', error);
    return res.status(500).json({ success: false, message: 'Error fetching tournament players' });
  }
};
...
export {
  ...
  checkEmailsRegistered, createGroupTeamPlayer, createIndividualTeamPlayer, getEventDetails,
