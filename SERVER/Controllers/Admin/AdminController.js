...
// ==================== PLAYER CONTROLLERS ====================
  ...
  // ==================== TOURNAMENT FEATURE TOGGLE ====================
  toggleFeaturedTournament: async (req, res) => {
    try {
      const { id } = req.params;
      ...
      res.status(200).json({
        success: true,
        message: `Tournament ${tournament.isFeatured ? "marked as" : "removed from"} featured`,
        data: tournament,
        toggledBy: req.admin?.name,
        timestamp: new Date().toISOString(),
      });
    } catch (error) {
      res.status(500).json({ success: false, message: "Server error while toggling featured status", error: error.message });
    }
  },
