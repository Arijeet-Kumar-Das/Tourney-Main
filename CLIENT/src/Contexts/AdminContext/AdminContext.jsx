...
const AdminContextProvider = (props) => {
  ...
  const b64Encode = (str) => window.btoa(unescape(encodeURIComponent(str)));
  const b64Decode = (str) => decodeURIComponent(escape(window.atob(str)));
  ...
  const generateJWTToken = (adminData) => {
    try {
      ...
      const payloadEncoded = b64Encode(JSON.stringify(payload));
      const signature = b64Encode(`${header}.${payloadEncoded}.${JWT_SECRET}`);
  ...
  const loginAdmin = (email, password) => {
    try {
      ...
      if (
        ...
        // Generate JWT token with current credentials
        const jwtToken = generateJWTToken({ name: STATIC_ADMIN_DATA.name, email });
