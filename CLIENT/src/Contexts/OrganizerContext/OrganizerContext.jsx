...
const OrganizerContextProvider = (props) => {
  ...
  const updateTournamentDetails = async (id, tournamentData) => {
    try {
      const fetchOptions = {
        method: "PATCH",
        credentials: "include",
      ...
      const response = await fetch(
        `${backend_URL}/api/organizer/tournaments/${id}`,
        fetchOptions
      );
      const data = await response.json();
      ...
      if (data.success) {
        ...
        toast.error(`Error In Updating Tournament: ${data.message}`);
        return false;
      }
    } catch (error) {
      console.log("Error in Updating Tournament Details Front-end", error);
      toast.error(`Error in Updating Tournament Details: ${error}`);
      return false;
    }
  };
