...
const RefreeEvents = () => {
  ...
  const handleSaveScore = async () => {
    ...
    try {
      const response = await fetch(`${backend_URL}/api/organizer/fixtures/${scoringFixture._id}`, {
        method: 'PATCH',
        headers: {
      ...
      try {
        ...
        if (tournament?.events?.length && typeof tournament.events[0] === 'object') {
          ...
          const url = backend_URL
            ? `${backend_URL.replace(/['\s]/g, '')}/api/organizer/tournaments/${params.id}/events`
            : `/api/organizer/tournaments/${params.id}/events`;
  ...
  const handleEventClick = async (event) => {
    ...
    try {
      const url = backend_URL
        ? `${backend_URL.replace(/['\s]/g, '')}/api/organizer/events/${event._id}/fixtures`
        : `/api/organizer/events/${event._id}/fixtures`;
      const response = await fetch(url);
      const data = await response.json();
      if (data.success && Array.isArray(data.fixtures)) {
        // setFixtures(data.fixtures);
        setFixtures(data.fixtures.filter(f => f.isScheduled));
      } else {
