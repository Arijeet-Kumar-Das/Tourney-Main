...
const Events = () => {
  ...
  const handleSubmitEvent = async(e) => {
    ...
    const missing = required.filter((f) => !newEventData[f]);
    if (missing.length) {
      toast.error('Please fill every field before submitting');
      return;
    }
  ...
  const handleEditEvent = (eventObj) => {
    setEditEventData({
      ...
      eventType2: eventObj.eventType2,
      matchType: eventObj.matchType,
      maxTeams: eventObj.maxTeams,
      entryFee: eventObj.entryFee,
      allowBooking: eventObj.allowBooking,
    });
  ...
  const handleUpdateEvent = async (e) => {
    ...
    try {
      const fetchOptions = {
        ...
        body: JSON.stringify({
          name: editEventData.eventName,
          eventType: editEventData.eventType,
          eventType2: editEventData.eventType2,
          matchType: editEventData.matchType,
          maxTeams: Number(editEventData.maxTeams),
          entryFee: Number(editEventData.entryFee),
          allowBooking: editEventData.allowBooking,
        }),
      };
      const response = await fetch(`${backend_URL}/api/organizer/events/${editEventData._id}`, fetchOptions);
      const data = await response.json();
      if (data.success) {
