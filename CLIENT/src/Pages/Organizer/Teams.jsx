...
const Teams = () => {
  ...
  const handleEditTeam = (teamObj) => {
    setEditTeamData(teamObj);
    setShowEditModal(true);
  };

  const handleTeamEditChange = (data) => {
  ...
  const handleUpdateTeam = async (e) => {
    ...
    try {
      let payload;
      if (eventType === 'group') {
        const { _id, __v, createdAt, updatedAt, ...rest } = editTeamData;
        payload = {
          teamName: rest.teamName,
          members: rest.members.map(({ _id, __v, ...m }) => m),
        };
      } else {
        const { _id, __v, createdAt, updatedAt, ...rest } = editTeamData;
        payload = {
          ...
          feesPaid: rest.feesPaid,
        };
      }
      const response = await fetch(`${backend_URL}/api/organizer/teams/${editTeamData._id}`, {
        method: 'PATCH',
        credentials: 'include',
              ...
              {filteredTeams?.map((team, index) => (
                <tr key={team._id} className="teams-table-row">
                  ...
                  <td className="teams-td teams-td-actions">
                    ...
                    >
                      <IoAdd style={{ transform: 'rotate(45deg)' }} />
                    </button>
                    <button
